function [tspan, X, denCond] = RA56_VDPL(fun,t0,tfin,x0,h0,hmin,hmax,abstol, reltol, mu)
X = x0;
x = x0;
tspan = t0;
h = h0;
...
while t < tfin - h
    [xnew, errR, ~, dCond] = RA56_step_VDPL (fun,t,h,x, mu);
...
h = tfin - t;
[x, ~, ~,dCond] = RA56_step_VDPL (fun,t,h,x, mu);
t = t + h;
tspan(i) = t;
denCond(i) = dCond;
X(:,i) = x;
